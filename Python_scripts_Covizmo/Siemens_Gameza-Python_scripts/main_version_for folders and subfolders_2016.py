...
for path, subdirs, files in os.walk(foldername):
    for name in files:
        if fnmatch(name, pattern):
            ...
            print(path)
            file2 = name[:-4]
            start = path_to_save.find('2016\\')# розділювач для створення структури папок
            print(start)
            ...
            try:
                    ...
                    data = data.rename(
                        columns={'No_CODE': 'No', 'ZONE_SHELL_ROOT': 'Code', 'val3': 'Description', 'val4': 'WorkToDo',
                                 'val5': 'BREAK REASON'})
                    ...
                    data['Length'] = data['val2'].str.split(' ').str.get(1)
                    data['Trans'] = data['val2'].str.split(' ').str.get(1)
                    data = data[
                    ...
                    damInfo = data[
                        ['WIND_FARM', 'FL', 'WTG', 'CLIENT', 'WO_No', 'SC', 'SC_RESPONSIBLE', 'SUPERVISOR', 'WTG_MODEL',
                         'COUNTRY', 'PRODUCTION_kWh', 'LINE_HOURS_OK', 'WTG_HOURS_OK', 'StartDate', 'EndDate',
